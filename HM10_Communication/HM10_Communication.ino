...
void setup() {
  ...
  Serial.begin(9600); // Start up the serial port so that we can see the incoming data in the serial monitor
  while(!Serial){     // Waiting till the serial port starts
    delay(500);
  }
...
void loop() {
  ...
  while(HM10_UART.available()){ // continously checking the HM10 to see if there are incoming data
    _recievedString += String((char)HM10_UART.read()); // read the data from HM10 as a String
